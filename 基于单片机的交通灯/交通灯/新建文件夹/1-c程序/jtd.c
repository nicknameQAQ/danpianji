...
uchar data sec_dx=15;  			//东西数默认值
uchar data sec_nb=15;			//南北数默认值
...
void main()
{
    ...
    sec_nb=sec_dx+5; 			//默认南北通行时间比东西多3秒
	while(1)					//主循环
	{	 
		key(); 					//调用按键扫描程序
		display(); 				//调用显示程序
...
void time0(void) interrupt 1 using 1  	//定时中断子程序 （发生在东西南北红绿灯交替时）
{
	...
	{	
		...
		if(sec_dx==0&&sec_nb==5) 		//当东西倒计时到0时，重置3秒，用于黄灯闪烁时间   
		{
			sec_dx=5;
			shanruo=1;
		}
		if(sec_nb==0&&sec_dx==5)		//当南北倒计时到0时，重置3秒，用于黄灯闪烁时间   
		{
			sec_nb=5;
			shanruo=1;
		}
...
void time1(void) interrupt 5 	//定时中断子程序 （用于黄灯闪烁）
{
	TH1=0X3C;							//重赋初值
