...
void PWM_EnablePDMA(PWM_T *pwm, uint32_t u32ChannelMask, uint32_t u32RisingFirst)
{
    ...
    for(i = 0; i < PWM_CHANNEL_NUM; i ++)
    {
        ...
        {
            u32IsOddCh = i % 2;
            (pwm)->PDMACTL = ((pwm)->PDMACTL & ~((PWM_PDMACTL_CHSEL0_1_Msk | PWM_PDMACTL_CAPORD0_1_Msk) << ((i >> 1) * 8))) | \
                             (((u32IsOddCh << PWM_PDMACTL_CHSEL0_1_Pos) | (u32RisingFirst << PWM_PDMACTL_CAPORD0_1_Pos) | \
                               PWM_PDMACTL_CAPMOD0_1_Msk | PWM_PDMACTL_CHEN0_1_Msk) << ((i >> 1) * 8));
        }
    }
}
...
void PWM_DisablePDMA(PWM_T *pwm, uint32_t u32ChannelMask)
{
    ...
    for(i = 0; i < PWM_CHANNEL_NUM; i ++)
    {
        if(u32ChannelMask & (1 << i))
        {
            (pwm)->PDMACTL &= ~(PWM_PDMACTL_CHEN0_1_Msk << ((i >> 1) * 8));
        }
    }
}

/**
 * @brief Enable Dead-time of selected channel
 * @param[in] pwm The base address of PWM module
 *                - PWM0 : PWM Group 0
 *                - PWM1 : PWM Group 1
 * @param[in] u32ChannelNum PWM channel number. Valid values are between 0~5
 * @param[in] u32Duration Dead-time length in PWM clock count, valid values are between 0~0xFFF, but 0 means there is no dead-time.
 * @return None
